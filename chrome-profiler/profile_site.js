...
const getDateTime = () => new Date().toISOString().replace(/[-:.]/g, '_');
const formatNumber = num => Number(num.toLocaleString('en-US', { maximumFractionDigits: 2 }));
const formatBytes = bytes => formatNumber(bytes / (1024 ** 2));
let child;
/**
...
async function pageProfiler(route, page) {
  ...
  // evaluateOnNewDocument is invoked after the document was
  // created but before any of its scripts were run
  await page.evaluateOnNewDocument(() => {
    ...
    XMLHttpRequest.prototype.open = (function _open(open) {
      return function _innerOpen(method, openUrl, async) {
        // https://developer.mozilla.org/en-US/docs/Web/API/XMLHttpRequest/open
        // async An optional Boolean parameter, defaulting to true
        if (async === false) {
          window.sync_async_xhr_total.sync += 1;
        } else {
          window.sync_async_xhr_total.async += 1;
        }
  ...
  const totalWatchers = await page.evaluate(() => {
  ...
  const totalGlobalVariables = await page.evaluate(() => Object.keys(window).length);
  const totalCache = await page.evaluate(() =>
    // https://stackoverflow.com/questions/4391575/how-to-find-the-size-of-localstorage
    // Each length is multiplied by 2 because the char in javascript stores as
    // UTF-16 (occupies 2 bytes)
    ...
    Object.keys(window.localStorage)
      ...
      + Object.keys(window.sessionStorage)
        .reduce((previous, key) => previous +
          (key.length + window.sessionStorage[key].length) * 2, 0));
...
async function hookProfiler() {
  ...
  try {
    ...
    if (config.puppeteerOptions.executablePath &&
        !fs.existsSync(config.puppeteerOptions.executablePath)) {
      let found = false;
    ...
    for (let i = 0; i < routes.length; i += 1) {
      try {
        const cpuCheck = setInterval(() => {
          pusage.stat(pid, { advanced: true }, (err, stat) => {
            if (err) {
              console.log(err);
            } else {
